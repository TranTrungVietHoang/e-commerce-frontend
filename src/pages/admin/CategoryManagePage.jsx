...
const CategoryManagePage = () => {
  ...
  const handleModalSubmit = async (values) => {
    try {
      // values.parentId có thể dùng để cấu trúc cây (tùy backend format Request)
      if (editingCat) {
  ...
  const openEditModal = (record) => {
    ...
    // Lưu ý form mapping với DB (parent là Object hay ID)
    form.setFieldsValue({
      name: record.name,
  ...
  return (
    <div style={{ padding: 24 }}>
      <Card 
        title={<Title level={4} style={{ margin: 0 }}><AppstoreOutlined /> Cây Danh Mục Sản Phẩm</Title>}
        extra={<Button type="primary" icon={<PlusOutlined />} onClick={openAddModal}>Thêm Mới</Button>}
      ...
      >
        <Table 
          ...
          loading={loading}
          pagination={false}
          // require backend trả về property `children` mảng thì Ant design Table sẽ tự show cây
        />
      </Card>
