      ...
      if (isAdminView) {
        ...
        if (!currentShopId) {
          try {
            ...
            console.error("Không thể xác định thông tin gian hàng", error);
          }
        }
        
        if (currentShopId) {
          fetchProducts(0, 10, currentShopId);
          fetchLowStock(currentShopId);
        }
      }
    };
    init();
  }, [isAdminView, user?.shopId]);
      ...
      <Card bordered={false} style={{ borderRadius: 12, boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
        ...
        {!isAdminView && !shopId && (
          <Alert
            ...
            description="Không tìm thấy cửa hàng của bạn. Vui lòng liên hệ hỗ trợ hoặc kiểm tra trạng thái đăng ký."
            type="error" showIcon
